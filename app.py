...
# =====================================================================
# ENDPOINTS DE OBSERVABILIDADE E MÉTRICAS
# =====================================================================
...
async def receive_metrics(request: Request):
    """
    Receber Web Vitals, Erros e Jornada do usuário
    ...
    try:
        ...
        logger.info(f"📊 Métricas recebidas - Session: {payload.get('sessionId')}")
        
        # Salvar em banco de dados (criar tabela se não existir)
        cursor.execute("""
        ...
        cursor.execute(
            """
            INSERT INTO metrics (session_id, metrics, errors, journey, user_agent, url)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (
                ...
                json.dumps(payload.get("journey", [])),
        ...
        for error in payload.get("errors", []):
            if error.get("type") == "js_error":
                logger.error(f"🔴 JS Error: {error.get('message')} - {error.get('filename')}")
...
async def receive_metrics_events(request: Request):
    """
    Receber eventos de conversão e comportamento
    
    Payload:
    {
      "sessionId": "123",
      "events": [
        {"name": "consulta_iniciada", "data": {}, "timestamp": ...},
        ...
      ],
      "funnel": {"visit": true, "consulta_iniciada": true, ...}
    }
    """
    try:
        payload = await request.json()
        ...
        for event in payload.get("events", []):
            cursor.execute(
                """
                INSERT INTO conversion_events (session_id, event_name, event_data, funnel_status)
                VALUES (?, ?, ?, ?)
                """,
                (
                    session_id,
                    ...
                    json.dumps(payload.get("funnel", {}))
                )
            )
        
        conn.commit()
        
        # Log conversão importante
        funnel = payload.get("funnel", {})
        if funnel.get("resultado_obtido"):
            logger.info(f"✅ Conversão: Resultado obtido - Session: {session_id}")
        if funnel.get("download_realizado"):
            logger.info(f"✅ Conversão: Download realizado - Session: {session_id}")
...
@app.post("/api/metrics/conversion")
async def track_conversion(request: Request):
    """
    Rastrear conversão rápida (usuário completou ação importante)
    """
    try:
        payload = await request.json()
        ...
        cursor.execute(
            "INSERT INTO conversions (session_id, conversion_type, value) VALUES (?, ?, ?)",
            (
                payload.get("sessionId"),
                payload.get("type"),
                payload.get("value", 1)
        ...
        logger.info(f"💰 Conversão: {payload.get('type')} - Value: {payload.get('value')}")
        
        return JSONResponse({"success": True})
    
    except Exception as e:
        logger.error(f"❌ Erro ao rastrear conversão: {e}")
        return JSONResponse({"error": str(e)}, status_code=400)
...
async def metrics_dashboard(request: Request):
    """
    Dashboard de métricas (admin only)
    """
    username = request.cookies.get("auth_user")
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(DISTINCT session_id) FROM metrics")
        ...
        cursor.execute("""
            ...
            FROM (
                SELECT json_extract(errors, '$[0].message') as error
                FROM metrics
                WHERE errors != '[]'
            )
            GROUP BY error
        ...
        # Taxa de conversão (resultado_obtido / consulta_iniciada)
        cursor.execute("""
            SELECT 
                COUNT(CASE WHEN event_name = 'consulta_iniciada' THEN 1 END) as consultas,
                COUNT(CASE WHEN event_name = 'resultado_obtido' THEN 1 END) as resultados
        ...
        return JSONResponse({
            ...
            "data": {
                ...
                "top_errors": [{"error": e[0], "count": e[1]} for e in top_errors] if top_errors else [],
                "conversions": [{"type": c[0], "count": c[1]} for c in conversions] if conversions else [],
                "conversion_rate": {
                    "consultas": conversion_rate[0] if conversion_rate else 0,
                    "resultados": conversion_rate[1] if conversion_rate else 0,
                    "percentage": round((conversion_rate[1] / conversion_rate[0] * 100) if conversion_rate[0] > 0 else 0, 2)
                }
            }
        })
