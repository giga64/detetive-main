...
if __name__ == "__main__":
    API_KEY = "876beb4baf6996f08b5149caa7fe5a7d"
    client = PortalTransparencia(api_key=API_KEY)
    
    print("\n" + "="*80)
    print("EXEMPLO 1: Buscar dados de servidor por CPF")
    print("="*80)
    servidor = client.buscar_servidor_por_cpf("453.178.287-91")
    if servidor:
        print(f"✅ Nome: {servidor.get('nome')}")
        print(f"   Tipo: {servidor.get('tipo_servidor')}")
        print(f"   Situação: {servidor.get('situacao')}")
        print(f"   Órgão: {servidor.get('orgao')}")
    else:
        print("❌ Nenhum dado encontrado")
    
    print("\n" + "="*80)
    print("EXEMPLO 2: Buscar informações de Pessoa Física")
    print("="*80)
    pessoa = client.buscar_dados_pessoa_fisica("453.178.287-91")
    if pessoa:
        print(f"✅ Nome: {pessoa.get('nome')}")
        print(f"   NIS: {pessoa.get('nis')}")
        if pessoa.get('envolvimentos'):
            print(f"\n   Envolvimentos:")
            for env in pessoa.get('envolvimentos'):
                print(f"     {env}")
        if pessoa.get('beneficios'):
            print(f"\n   Benefícios Sociais:")
            for ben in pessoa.get('beneficios'):
                print(f"     • {ben}")
        if pessoa.get('sancoes'):
            print(f"\n   ⚠️ Sanções:")
            for sanc in pessoa.get('sancoes'):
                print(f"     {sanc}")
    else:
        print("❌ Nenhuma informação encontrada")
    
    print("\n" + "="*80)
    print("EXEMPLO 3: Buscar Bolsa Família para um CPF")
    print("="*80)
    beneficios = client.buscar_bolsa_familia_por_cpf("453.178.287-91")
    if beneficios:
        print(f"✅ {len(beneficios)} benefício(s) encontrado(s)")
        for benef in beneficios[:3]:
            print(f"   {benef.get('mes_ano')}: {benef.get('valor_formatado')}")
    else:
        print("❌ Nenhum benefício encontrado")
    
    print("\n" + "="*80)
    print("EXEMPLO 4: Buscar convênios para um CNPJ")
    print("="*80)
    convenios = client.buscar_convenios_por_cnpj("09.464.032/0001-12")
    if convenios:
        print(f"✅ {len(convenios)} convênio(s) encontrado(s)")
        for conv in convenios[:3]:
            print(f"   {conv.get('nome')}: {conv.get('valor_formatado')}")
    else:
